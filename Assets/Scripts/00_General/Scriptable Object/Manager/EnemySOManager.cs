...
{
	public class EnemySOManager : SOManager<EnemySOManager>
	{
		static EnemySOManager()
		{
			dataFolder = "EnemyData";
		...
		public override void CreateSO()
		{
			...
			DataTable dataTable = DataUtil.GetDataTable("EnemyData", "적");

			if (dataTable == null)
			{
				Debug.LogError("올바른 Enemy 데이터 테이블이 존재하지 않습니다.");
				return;
			}
			...
			if (rows.Length < 1)
			{
				Debug.LogWarning("Enemy 테이블은 존재하나 Enemy 데이터 없음");
				return;
			}
			...
			for (int i = 0; i < rows.Length; ++i)
			{
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [코드] 전환 오류!\n" +
						"코드: " + codeStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [체력] 전환 오류!\n" +
						"체력: " + hpStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [체력 회복량] 전환 오류!\n" +
						"체력 회복량: " + hpRegenStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [체력 재생 쿨타임] 전환 오류!\n" +
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [힐 배율] 전환 오류!\n" +
						"힐 배율: " + healScaleStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [치유 감소 배율] 전환 오류!\n" +
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [방어력] 전환 오류!\n" +
						"방어력: " + armorStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [공격력] 전환 오류!\n" +
						"공격력: " + attackPowerStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [공격 속도] 전환 오류!\n" +
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [공격 크기] 전환 오류!\n" +
						"공격 크기: " + attackSizeStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [투사체 속도] 전환 오류!\n" +
						"투사체 속도: " + shotSpeedStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [투사체 생존 시간] 전환 오류!\n" +
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [타격 수] 전환 오류!\n" +
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [치명타 확률] 전환 오류!\n" +
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [치명타 대미지 배율] 전환 오류!\n" +
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [회피율] 전환 오류!\n" +
						"회피율: " + avoidabilityStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [이동 속도] 전환 오류!\n" +
						"이동 속도: " + moveSpeedStr);
					return;
				}
				#endregion
				...
				{
					Debug.LogError(titleStr + " 적 데이터 [시야 거리] 전환 오류!\n" +
						"시야 거리: " + sightStr);
					return;
				}
				#endregion
				#region 경험치
				string xpStr = row[20] as string;

				if (float.TryParse(xpStr, out float xp) == false)
				{
					Debug.LogError(titleStr + " 적 데이터 [경험치] 전환 오류!\n" +
						"경험치: " + xpStr);
					return;
				}
				#endregion
				#region 경험치 배율
				string xpScaleStr = row[21] as string;

				if (float.TryParse(xpScaleStr, out float xpScale) == false)
				{
					Debug.LogError(titleStr + " 적 데이터 [경험치 배율] 전환 오류!\n" +
						"경험치 배율: " + xpScaleStr);
					return;
				}
				#endregion
				...
				data.Initialize(assetPathStr, code, titleStr, hp, hpRegen, hpRegenTime, healScale, antiHealScale, armor, attackPower, attackSpeed, attackSize, shotSpeed, attackRange, multiHitCount, criticalRate, criticalDamageScale, avoidability, moveSpeed, sight, xp, xpScale);
			...
			Debug.Log("Enemy 스크립터블 오브젝트 생성 완료");
		}
